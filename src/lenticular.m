...
EI_sizey = Display_npy;
EI = (zeros(EI_sizey, EI_sizex,3,'uint8'));
Ref_img = imresize(img,[EI_sizey, EI_sizex]);
Ref_depth = imresize(depth,[EI_sizey, EI_sizex]);
Ref_img = padarray(Ref_img,[0, Shift_max]);
Ref_depth = padarray(Ref_depth,[0, Shift_max]);

% Sort %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fprintf("Number of lenslet Col:%d\n", ceil(Lens_x));
for x = 1:ceil(Lens_x)
    fprintf("Col: %d/%d\n", x, ceil(Lens_x));
    %range_ei = 1+(x-1)*Lens_psx:x*Lens_psx;
    range_masked = 1+(x-1)*Lens_psx:x*Lens_psx+Shift_max*2; % Calculate maske range of x axis
    for y = 1:Display_npy
        part_depth = Ref_depth(y,range_masked,1);       % Region selection by depth
        part_depth_pos = find(part_depth>0);            % find depth>0
        dep = part_depth(part_depth_pos);               % dep is depth list
        [deps, index] = sort(dep);                      % depth sort
        
        LUT_list = part_depth_pos(index) + (double(deps)-1)*LUT_size;   % Convert to 1D access for LUT(faster than 2D access)
        rewrite = LUT(LUT_list);                        % rewrite means LUT's return value(Lenslet's screen position)
        ...
        ref_pos = 1+(x-1)*Lens_psx + part_depth_pos(index(rewrite_pos));    % Refarence image's position list (x axis)
        ei_pos = (x-1)*Lens_psx+ rewrite(rewrite_pos);                      % Elemental image's position list (x axis)
        EI(y,ei_pos,:) = Ref_img(y,ref_pos,:);                              % image sort
    end
end
imshow(EI);
mkdir('../out/image/');
imwrite(gather(EI(1:Display_npy,1:Display_npx,:)),['../out/image/Lenticular_' num2str(Display_inch) '_' num2str(Display_npx) 'x' num2str(Display_npy) '_' num2str(Z_start) '_' num2str(Z_end) '.png']);
